...
public class FrmContact extends javax.swing.JFrame {
    ...
    dateOfBirth.set(2005, Calendar.APRIL, 10);

    Contact contact = new Contact(id, name, sex, gender, maritalStatus, dateOfBirth, favoriteSports, comments);

    boolean isSaved;
    isSaved = MongoDBManager.addContact(contact);
    if (isSaved) {
    JOptionPane.showMessageDialog(rootPane, "Successfully saved: " + contact.toString());
    } else {
    ...
    Contact contactDocument = MongoDBManager.findContact(searchId);

    if (contactDocument != null) {
        txtName.setText(contactDocument.getName());
        cmbSex.setSelectedItem("m".equals(contactDocument.getSex()) ? "male" : "female");
        cmbGender.setSelectedItem(contactDocument.getGender());
        cmbMaritalStatus.setSelectedItem(contactDocument.getMaritalStatus());
        txaComments.setText(contactDocument.getComments());
