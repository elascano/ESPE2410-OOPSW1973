...
class AddStudentView:
    def __init__(self, root, return_to_menu_callback):
        ...
        self.grades_label = tk.Label(root, text="Grades (comma separated)")
        self.grades_label.pack(pady=5)
        self.grades_entry = tk.Entry(root)
        self.grades_entry.pack(pady=5)
    ...
    def add_student(self):
        try:
            grades = list(map(int, self.grades_entry.get().split(",")))
        except ValueError:
            messagebox.showerror("Error", "Invalid grades input. Please enter comma-separated numbers.")
            return
        student = Student(self.id_entry.get(), self.name_entry.get(), self.lastName_entry.get(), grades)
