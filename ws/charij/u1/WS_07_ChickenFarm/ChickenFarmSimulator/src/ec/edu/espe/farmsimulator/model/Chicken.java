...
public class Chicken {
    ...
    public int computeAgeInMonths() {
        if (BornOnDate == null) {
        return 0; // Si la fecha de nacimiento es nula, no se puede calcular la edad.
    }
    ...
    // Ajustar la diferencia total en meses
    int totalMonths = yearsDifference * 12 + monthsDifference;

    // Si el día actual es anterior al día de nacimiento, restar un mes
    if (currentDate.get(Calendar.DAY_OF_MONTH) < birthDate.get(Calendar.DAY_OF_MONTH)) {
        totalMonths--;
    }

    return totalMonths < 0 ? 0 : totalMonths; // Asegurarse de que no sea negativo
}
